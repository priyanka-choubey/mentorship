...
from authentication.choices import EDUCATION_CHOICES, MENTORSHIP_AREAS_CHOICES
...
class ProfileForm(forms.ModelForm):
    ...
    phone_number = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control'}),max_length=50,required=False)
    email = forms.EmailField(widget=forms.TextInput(attrs={'class': 'form-control'}),max_length=75,required=False)
    ...
    mentorship_areas = forms.MultipleChoiceField(widget=forms.CheckboxSelectMultiple(), choices=MENTORSHIP_AREAS_CHOICES)
    highest_level_of_study = forms.ChoiceField(widget=forms.Select(attrs={'class': 'form-control'}),choices=EDUCATION_CHOICES)
    ...
    class Meta:
        ...
        widgets = {
            'bio': forms.Textarea(
                attrs={'cols': 20, 'rows': 10, 'class': 'form-control'}),
        }
...
class ContactForm(forms.Form):
    sender = forms.EmailField(max_length = 225, required =True, label = '',widget = forms.EmailInput(attrs={'class': 'form-control','placeholder':'Your Email', 'required':'true'}))
    subject = forms.CharField(max_length = 100, required =True, label='',widget = forms.TextInput(attrs={'class': 'form-control','placeholder':'Subject', 'require':'true'}))
    message = forms.CharField(widget = forms.Textarea(attrs={
        'class': 'form-control',
        'rows': 3,
        'placeholder':'Let us know how we can help!', 'required':'true'
        }),
        required =True, label='')
